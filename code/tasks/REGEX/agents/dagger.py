...
class DaggerStudent(object):
    ...
    def act(self, gold_actions=None, pred_actions=None, sample=False, debug=False):
        ...
        if self.is_eval:
            ...
            pred_actions = [self.tgt_vocab[w] for w in pred_actions]
            self.prev_actions = self._to_tensor(pred_actions).long()
    ...
    def receive(self, src_words, instructions, pred_regexes, teacher_action_seqs):

        assert len(src_words) == len(teacher_action_seqs)

        self.init(src_words, instructions, False)
        ...
        while not self.has_terminated():
            ...
            gold_actions = [None] * batch_size
            for i in range(batch_size):
                if t + 1 < len(teacher_action_seqs[i]):
                    gold_actions[i] = teacher_action_seqs[i][t + 1]
                else:
                    gold_actions[i] = '<PAD>'

                if t + 1 < len(pred_regexes[i]):
                    pred_actions[i] = pred_regexes[i][t + 1]
                else:
                    pred_actions[i] = '<PAD>'

            self.act(gold_actions=gold_actions, pred_actions=pred_actions)
            t += 1
