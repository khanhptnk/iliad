...
class ReinforceStudent(object):
    ...
    def receive(self, src_words, instructions, pred_regexes, rewards):

        assert len(src_words) == len(rewards)
        ...
        while not self.has_terminated():
            ...
            for i in range(batch_size):
                if t + 1 < len(pred_regexes[i]):
                    actions[i] = pred_regexes[i][t + 1]
                else:
                    actions[i] = '<PAD>'
            self.act(gold_actions=actions)
            t += 1
