...
class DaggerStudent(object):
    ...
    def act(self, states, teacher_actions=None, student_actions=None):
    ...
    def receive(self, init_poses, instructions, pred_paths, ref_action_seqs):

        assert len(pred_paths) == len(ref_action_seqs)
        ...
        while not self.has_terminated():
            ...
            for i, state in enumerate(states):
                ...
                if t >= len(ref_action_seqs[i]):
                    gold_actions[i] = self.STOP
                else:
                    gold_actions[i] = ref_action_seqs[i][t]

            self.act(states, teacher_actions=gold_actions,
                             student_actions=pred_actions)
            states = states.step(pred_actions)
            t += 1
